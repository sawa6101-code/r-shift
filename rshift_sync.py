...
def parse_staff_month(html, year, month, staff_name):
    ...
    for idx, node in enumerate(shift_cols[:days]):
        ...
        if "help_shift" in classes:
            ...
            if not shift:
                continue
            item = (shift[0], shift[1], "店舗勤務", NORMAL_WORKPLACE, False)
...
def home_coordinates():
    """GitHub Secretsの緯度・経度を優先し、未設定時だけ住所へフォールバックする。"""
    ...
    raise RuntimeError("RSHIFT_HOME_LAT / RSHIFT_HOME_LON が未設定で、RSHIFT_HOME_ADDRESSからも自宅位置を取得できませんでした")
...
def add_travel_events(cal, shifts):
    ...
    for start, end, summary, location, is_help in shifts:
        ...
        event = Event()
        uid_source = f"travel|{departure.isoformat()}|{start.isoformat()}|{destination_name}|{minutes}|{TRAVEL_BUFFER_MINUTES}"
        ...
        event.add("location", destination_name)
        event.add("X-RSHIFT-ORIGIN", "自宅")
        event.add("X-RSHIFT-DESTINATION", destination_name)
        event.add("X-RSHIFT-TRAVEL-MINUTES", str(minutes))
        event.add("X-RSHIFT-TRAVEL-BUFFER-MINUTES", str(TRAVEL_BUFFER_MINUTES))
